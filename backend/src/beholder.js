...
module.exports = class Beholder {
    ...
    async pingTuning(pingAutomation) {
        const exchange = new Exchange();
        const startTime = Date.now();
        const { serverTime } = await exchange.time();
        const difference = Math.floor((serverTime - startTime) * 0.8);

        if (pingAutomation.logs)
            logger("A:" + pingAutomation.id, `PING: The schedule was ${pingAutomation.schedule}, but the ping 80% difference is ${difference}. I'll adjust!`);

        const automationId = typeof pingAutomation.id === "string" ? Number(pingAutomation.id.replace("P", "")) : pingAutomation.id;
        const orderAutomation = await automationsRepository.getAutomation(automationId);

        orderAutomation.schedule = new Date(Date.parse(orderAutomation.schedule) - difference).toISOString();
        await automationsRepository.updateAutomation(orderAutomation.id, { schedule: orderAutomation.schedule });

        return { text: `Ping tested and ajusted successfully with id #${pingAutomation.id}!`, type: "success" };
    }
    ...
    doAction(settings, user, action, automation) {
        try {
            switch (action.type) {
                ...
                case actionTypes.PING: return this.pingTuning(automation);
            }
        } catch (err) {
            if (automation.logs) {
    ...
    shouldntInvert(automation, memoryKey) {
        if (!automation.actions || !automation.actions.length) return false;
    ...
    async checkActivation(automation, memoryKey) {
        ...
        const hasVaribalesInMemory = indexes.every(ix => MEMORY[ix] !== null && MEMORY[ix] !== undefined);
        if (!hasVaribalesInMemory) return false;
