...
async function sendResetAlerts(user, newPassword) {
    ...
    if (settings.sendGridKey)
        ...
        Enjoy!
        `, user.email, 'Beholder password resetted!'));
...
async function sendWelcomeAlerts(user, newPassword) {
    ...
    if (settings.sendGridKey)
        ...
        Enjoy!
        `, user.email, 'Beholder account created!'));
...
async function sendStopAlerts(user) {
    ...
    if (settings.sendGridKey)
        promises.push(email(settings, fullText, user.email, 'Beholder account stopped!'));

    const shortText = `Your account at Beholder was stopped. Sorry!`;

    if (user.phone && settings.twilioSid)
        promises.push(sms(settings, shortText, user.phone));

    if (user.pushToken)
        promises.push(push.send(user, shortText, 'Beholder Notification', {
            text: shortText,
            type: 'error'
        }));
...
async function sendStartAlerts(user) {
    ...
    if (settings.sendGridKey)
        promises.push(email(settings, fullText, user.email, 'Beholder account started!'));

    const shortText = `Your account at Beholder was (re)started. Enjoy!`;
